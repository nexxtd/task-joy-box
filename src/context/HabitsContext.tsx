      ...
      if (parsed?.columns) {
        ...
        void (async () => {
          try {
            ...
            // Do NOT blindly overwrite localStorage with server data here.
          } catch {}
        })();
...
async function saveBoard(userId: number, board: Board, retryCount = 0): Promise<boolean> {
  ...
  try {
    ...
    const response = await fetch('/api/habit-boards/snapshot', {
      signal: ac.signal,
      method: 'POST',
    ...
    const handleVisibilityChange = async () => {
      if (document.visibilityState === 'visible') {
        ...
        try {
          ...
          const res = await fetch('/api/habit-boards/snapshot', { credentials: 'include', signal: vc.signal });
          clearTimeout(tid);
          if (res.ok) {
            const data = await res.json();
