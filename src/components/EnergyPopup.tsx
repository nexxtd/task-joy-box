...
const EnergyPopup: React.FC = () => {
  ...
  const saveSlot = useCallback(() => {
    ...
    const updated: EnergyEntry = {
      ...
      note: existing?.note,
      [currentSlot]: selectedLevel,
    };
    const next = [...entries.filter(e => e.date !== today), updated].sort((a, b) => a.date.localeCompare(b.date));
    saveHistory(next);

    const pending = computePending().filter(s => s !== currentSlot);
    if (pending.length > 0) {
      setCurrentSlot(pending[0]);
      setSelectedLevel('medium');
    } else {
      setCurrentSlot(null);
    }
  }, [currentSlot, selectedLevel, computePending]);
  ...
  useEffect(() => {
    const tick = () => {
      ...
      if (currentSlot === 'morning' && h >= 12 && lastHourRef.current < 12) {
      ...
      if (pending.length > 0) {
        setCurrentSlot(prev => {
          if (prev && pending.includes(prev)) return prev;
          return pending[0];
        });
      } else {
        setCurrentSlot(null);
  ...
  }, [getTodayEntry, currentSlot]);

  if (!enabled || !currentSlot) return null;

  const slotDef = SLOTS.find(s => s.id === currentSlot);
  const today = localDayKey();
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
        <div className="flex items-center justify-center gap-2 mb-1">
          ...
          <h3 className="text-base font-bold text-foreground">{slotDef?.checkLabel}</h3>
        </div>
        ...
        <button
          ...
          className="w-full py-2.5 bg-primary text-primary-foreground text-sm font-bold rounded-xl hover:bg-primary/90 transition-all shadow-lg shadow-primary/20 active:scale-[0.98] disabled:opacity-50"
        >
          Save
        </button>
