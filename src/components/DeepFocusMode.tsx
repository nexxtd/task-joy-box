              ...
              <div className="rounded-2xl border border-border bg-muted/20">
                ...
                {!attachmentsCollapsed && (
                  <div className="border-t border-border/60 px-4 py-3 space-y-3">
                    ...
                    {(selectedTask.attachments?.length ?? 0) > 0 ? (
                      <FreeAttachmentList attachments={selectedTask.attachments || []} taskId={selectedTask.id} taskTitle={selectedTask.title} onDelete={deleteAttachment} onReorder={(items)=>updateTask(selectedTask.id,{attachments: items})} />
                    ) : null}
                  </div>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-border bg-muted/20">
                ...
                {!imagesCollapsed && (
                  <div className="border-t border-border/60 px-4 py-3 space-y-3">
                    ...
                    {(selectedTask.images?.length ?? 0) > 0 ? (
                      <DraggableImageGrid images={selectedTask.images || []} onReorder={(items)=>updateTask(selectedTask.id,{images: items})} onRemove={deleteImage} droppableId={`deepfocus-images-${selectedTask.id}`} />
                    ) : null}
                  </div>
                )}
              </div>
