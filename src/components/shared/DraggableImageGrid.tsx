...
export const DraggableImageGrid: React.FC<DraggableImageGridProps> = ({
  ...
  const getInsertIndex = useCallback((clientX: number, clientY: number, draggedId: string) => {
    ...
    if (targetEl) {
      ...
      const cx = r.left + r.width / 2;
      const cy = r.top + r.height / 2;
      const isAfter = clientY > cy + 4 || (Math.abs(clientY - cy) < 20 && clientX > cx);
  ...
  return (
    <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 select-none ${dragId ? 'cursor-grabbing' : ''}`}>
      {display.map((img) => {
        ...
        return (
          <div
            ...
            className={`relative group/img aspect-square rounded-xl border bg-muted/40 overflow-hidden transition-all duration-200 ${isDragged ? 'opacity-40 border-dashed border-primary/50 scale-[0.98] z-0' : 'hover:border-primary/40 hover:shadow-md border-border'}`}
            style={{ transition: isDragged ? 'none' : 'transform 200ms cubic-bezier(0.22,1,0.36,1), opacity 150ms' }}
          >
            <div
      ...
      {dragId && ghostPos && (() => {
        ...
        return (
          <div className="fixed pointer-events-none z-50 rounded-xl overflow-hidden shadow-2xl ring-2 ring-primary border-primary/50 opacity-90 scale-105" style={{ left: ghostPos.x, top: ghostPos.y, width: ghostPos.w, height: ghostPos.h }}>
