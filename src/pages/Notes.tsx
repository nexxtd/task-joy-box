        ...
        <div className="flex items-center gap-1 px-3 py-3">
          ...
          {!isDeleteMode && (
            <div className="flex items-center gap-1.5 flex-shrink-0">
              ...
              <button
                onClick={e => { e.stopPropagation(); openDeepFocus(note); }}
                    ...
                    {filtered.completed.map(note => (
                      ...
                      >
                        ...
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-label-green/15 text-label-green font-medium flex-shrink-0">
                          Auto-delete in {daysUntilAutoDelete(note.completedAt)} day{daysUntilAutoDelete(note.completedAt) === 1 ? '' : 's'}
                        </span>
                        <button
                          onClick={e => { e.stopPropagation(); setSingleDeleteTaskId(note.id); }}
                ...
                <>
                  ...
                  <div className="absolute left-0 mt-2 w-96 max-w-[95vw] bg-card border border-border rounded-2xl shadow-xl z-30 p-4 space-y-3">
                    <div className="max-h-52 overflow-y-auto space-y-1.5 pr-1">
                      ...
                      {allTags.map(tag => {
                        const active = newNoteLabels.some(l => l.id === tag.id);
                        return (
                          <button
                            key={tag.id}
                            onClick={() => setNewNoteLabels(prev => active ? prev.filter(l => l.id !== tag.id) : [...prev, tag])}
                            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all text-left ${
                              active
                                ? 'border-primary/30 bg-primary/5 shadow-sm'
                                : 'border-border/60 hover:bg-muted/40'
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setTagPopupTaskId(null)}>
            ...
            <div className="relative w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div className="max-h-60 space-y-2 overflow-y-auto mb-4">
                {allTags.map(label => {
                  const active = popupNote.labels.some(item => item.id === label.id);
                  return (
                    <div key={label.id} className="flex items-center gap-2 rounded-xl border border-border/60 px-3 py-2">
                      <button onClick={() => toggleNoteTag(popupNote.id, label)} className="flex flex-1 items-center gap-2 text-left">
              ...
              <div className="border-t border-border pt-4">
                ...
                <div className="flex gap-2">
                  <button onClick={async () => {
                    ...
                    if (!name) return;
                    try {
                      const label = await createSharedNoteLabel(name, newTagColor);
          ...
          {note.labels.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {note.labels.map(label => (
                <button
                  key={label.id}
                  onClick={() => onToggleTag(note.id, label)}
          ...
          {tagPickerOpen && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setTagPickerOpen(false)}>
              ...
              <div className="relative w-full max-w-lg rounded-2xl border border-border bg-card p-5 shadow-2xl" onClick={e => e.stopPropagation()}>
                ...
                <div className="max-h-60 space-y-2 overflow-y-auto mb-4">
                  {allTags.map(label => {
                    const active = note.labels.some(item => item.id === label.id);
                    return (
                      <div key={label.id} className="flex items-center gap-2 rounded-xl border border-border/60 px-3 py-2">
                        <button onClick={() => onToggleTag(note.id, label)} className="flex flex-1 items-center gap-2 text-left">
                          <span className={`w-3 h-3 rounded-full ${LABEL_COLORS[label.color]}`} />
                          <span className="text-sm text-foreground">{label.name}</span>
                          {active && <span className="ml-auto text-[10px] text-primary font-semibold">Selected</span>}
                        </button>
                ...
                <div className="border-t border-border pt-4">
                  ...
                  <div className="flex gap-2">
                    <button onClick={createTagForNote} disabled={!newTagName.trim()} className="flex-1 rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground disabled:opacity-50">Add tag</button>
