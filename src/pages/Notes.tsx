...
const Tasks: React.FC = () => {
  ...
  useEffect(() => { localStorage.setItem('notes-mynotes-collapsed', String(myTasksCollapsed)); }, [myTasksCollapsed]);
  ...
  const [collapsedCompletedSections, setCollapsedCompletedSections] = useState<Record<string, boolean>>(() => {
    try { const v = localStorage.getItem('notes-collapsed-completed'); return v ? JSON.parse(v) : {}; } catch { return {}; }
  });
  useEffect(() => { try { localStorage.setItem('notes-collapsed-completed', JSON.stringify(collapsedCompletedSections)); } catch {} }, [collapsedCompletedSections]);
  ...
  const getTasksForDroppable = (id: string): Task[] | null => {
    if (id === 'my-notes') return myTasksGroup;
    if (id.startsWith('col-')) {
  ...
  const expandDroppableGroup = (droppableId: string) => {
    if (droppableId === 'my-notes') { setMyTasksCollapsed(false); return; }
        ...
        <div className="max-w-5xl mx-auto space-y-2 pb-24">
          ...
          {(myTasksGroup.length > 0 || filtered.completed.some(t => !t.projectId)) && (
            <div className="mb-3">
              ...
              <Droppable droppableId="my-notes" renderClone={renderTaskClone}>
              ...
              {!myTasksCollapsed && renderCompletedSection('my-notes', filtered.completed.filter(t => !t.projectId))}
            </div>
          )}
          ...
          >
            ...
            {!isPro ? (
              <div className="p-6 text-center space-y-4">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mx-auto">AI Note Builder is available exclusively for Pro users. Upgrade to unlock AI-powered task creation.</p>
                <button
                  onClick={() => window.location.href = '/pricing'}
