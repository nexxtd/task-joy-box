...
import { db } from '../db';
import { noteSnapshots, tasks, noteBoards, users } from '../../shared/schema';
...
router.get('/snapshot', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [snapshot] = await db.select()
      .from(noteSnapshots)
      .where(eq(noteSnapshots.userId, userId))
...
router.post('/snapshot', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await db.delete(noteSnapshots).where(eq(noteSnapshots.userId, userId));

    await db.insert(noteSnapshots).values({
      userId,
...
router.get('/tasks/:projectId/assignments', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const projectnoteBoards = await db.select({ id: noteBoards.id })
      .from(noteBoards)
      .where(eq(noteBoards.userId, req.userId!));
    
    const boardIds = projectnoteBoards.map(b => b.id);
