...
import { habitTemplates } from '../../shared/schema.js';
...
router.get('/', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const result = await db.select().from(habitTemplates).where(eq(habitTemplates.userId, req.userId!)).orderBy(habitTemplates.name);
...
router.put('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await db.select().from(habitTemplates).where(eq(habitTemplates.id, id)).limit(1);
    if (!existing.length || existing[0].userId !== req.userId) return res.status(404).json({ error: 'Template not found' });
    const { name, title, description, priority, duration, startDate, startTime, dueDate, dueTime, projectId, columnId, labels, subtasks, checklists, images, attachments } = req.body;
    const [updated] = await db.update(habitTemplates).set({
    ...
    }).where(eq(habitTemplates.id, id)).returning();
...
router.delete('/:id', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await db.select().from(habitTemplates).where(eq(habitTemplates.id, id)).limit(1);
    if (!existing.length || existing[0].userId !== req.userId) return res.status(404).json({ error: 'Template not found' });
    await db.delete(habitTemplates).where(eq(habitTemplates.id, id));
