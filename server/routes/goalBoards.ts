...
import { goalSnapshots, users } from '../../shared/schema.js';
...
router.get('/snapshot', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    const snapshot = await db.select()
      .from(goalSnapshots)
      .where(eq(goalSnapshots.userId, req.userId!))
    ...
    try {
      ...
      console.error('Failed to parse goal board snapshot');
      res.json({ board: null });
    }
  } catch (error) {
...
router.post('/snapshot', requireAuth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await db.insert(goalSnapshots)
      .values({
        userId: req.userId!,
    ...
    const userSnapshots = await db.select({ id: goalSnapshots.id })
      .from(goalSnapshots)
      .where(eq(goalSnapshots.userId, req.userId!))
      .orderBy(desc(goalSnapshots.updatedAt));
    ...
    if (userSnapshots.length > 10) {
      ...
      if (idsToDelete.length > 0) {
        await db.delete(goalSnapshots).where(inArray(goalSnapshots.id, idsToDelete));
      }
    }
